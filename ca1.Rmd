...
#no ties
plot(no_ties$t0, no_ties$Ahat, type = "s", xlab = "t", ylab = "Cumulative hazard", col = "blue")
lines(no_ties$t0,  no_ties$KMAhat, type = "s", xlab = "t", col = "red")
...
#with ties
plot(with_ties$t0, with_ties$Ahat, type = "s", xlab = "t", ylab = "Cumulative hazard", col = "blue")
lines(with_ties$t0,  with_ties$KMAhat, type = "s", xlab = "t", col = "red")
...
#number of censored events in bins of true events
c0 <- ((data.frame(t0 = t0) %>% 
  mutate(cens = as.numeric(c<t0)) %>%   
  mutate(t0_bin = round(t0 + 0.05, digits = 1)) %>% 
  arrange(t0) %>% 
  filter(cens == 1))$t0 %>% cut(breaks = c(0,with_censor$t0)) %>% table() %>% as.data.frame())$Freq
...
with_censor <- with_censor %>% 
  mutate(c0 = c0) %>% 
  # mutate(temp = d0 + c0) %>% 
  # mutate(y0 = cumsum(rev((with_censor %>% mutate(c0 = c0) %>% mutate(temp = d0+c0))$temp)) %>% rev()) %>% 
  mutate(y0 = y0) %>%
  mutate(Ahat = cumsum(d0 / y0)) %>% 
  mutate(var = cumsum(1/(y0^2))) %>% 
  mutate(lower = Ahat*exp(-qnorm(0.95) * sqrt(var)/ Ahat)) %>% 
...
#km
plot(no_ties$t0, no_ties$KMAhat, type = "s", xlab = "t", ylab = "Cumulative hazard")
lines(with_ties$t0, with_ties$KMAhat, type = "s", col = "red")
lines(with_censor$t0, with_censor$KMAhat, type = "s", col = "blue")
lines(with_censor$t0, with_censor$lowerKM, type = "s", lty = "dashed", col = "blue")
lines(with_censor$t0, with_censor$upperKM, type = "s", lty = "dashed", col = "blue")
...
lines(seq, weibull_haz(seq, 4.5, 22.5), col = "green")
